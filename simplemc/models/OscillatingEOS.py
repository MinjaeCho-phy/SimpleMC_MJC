...
from simplemc.cosmo.paramDefs import w1_par, w2_par, b_oscil_par, ac_par

import matplotlib.pyplot as plt

class OscillatingEOS(LCDMCosmology):
    def __init__(self, varyw1=True, varyw2=True, varyb_oscil=True, varyac=True, model = 'model1'):
        self.varyw1  = varyw1
        self.varyw2  = varyw2
    ...
    def freeParameters(self):
        ...
        if (self.varyac): l.append(ac_par)
        return l
    ...
    def de_eos(self, z):
        ...
        elif self.model == 'model10':
            w = self.w1 + self.w2*np.cos(self.b*np.log((1.0/(1.0+z))/self.ac))
        elif self.model == 'model11':
            w = self.w1 + self.w2*(1./(1.+z))*np.cos(self.b*(1./(1.+z))+self.ac)
        elif self.model == 'model12':
            w = self.w1 - selfw2*np.sin(self.b*np.log((1./(1.+z)))+self.ac)
        return w
    ...
    def de_rhow(self, z):
        ...
        resultado = quad(lambda b: 3.0*(1.0+ self.de_eos(b))/(1.0+b), 0.0, z )
        return resultado[0]
